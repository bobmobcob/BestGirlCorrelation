...
public class BestGirlCorrelationVHigi2{
	...
	public static void main( String[] args ){
		...
		for( CharacterLink oneCharacter : allCharacters ){
			if( oneCharacter.getVotes() > numOfVotesForGirlToCount ){//to avoid girls with only a few votes skewing data
				for (Map.Entry<CharacterLink, Integer> entry : oneCharacter.mapEntrySet()) {
					arrListForSortResults.add(new CorrelationPair( ((double)(Math.round(100000.0* (((entry.getValue()+0.0)/((oneCharacter.getVotes()+0.0+entry.getKey().getVotes())/2)) - (((double)entry.getKey().getVotes()+(double)oneCharacter.getVotes())/2)/TotalVotes) )))/1000.0, oneCharacter.getName(), entry.getKey().getName()));
				}
			}
		}
		...
		for( CorrelationPair oneResult : arrListForSortResults ){
			...
			if(!alreadyPrinted.contains(oneResult.getFirstCharacter()+oneResult.getSecondCharacter())){
				//oneResult.printPair();
				alreadyPrinted.add(oneResult.getSecondCharacter()+oneResult.getFirstCharacter());
				System.out.println(""+Math.round(100*oneResult.getDouble())+"\t"+oneResult.getSecondCharacter().replaceFirst(";",",")+"\t"+oneResult.getFirstCharacter().replaceFirst(";",","));
			}
			//}
		}
	}//end main
